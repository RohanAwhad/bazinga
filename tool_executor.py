    ...
    def search_replace(
        ...
        *,
        ...
        if not p.exists():
            if create_if_missing and old_string == "":
                ...
                if rel and self._is_artifact_path(rel):
                    # Strip .dingllm/ prefix since save_artifact scopes to .dingllm/ internally
                    artifact_rel = rel.removeprefix(".dingllm/")
                    return self._artifact_persistence.save_artifact(
                        project_path, artifact_rel, new_string
                    )
        ...
        if rel and self._is_artifact_path(rel):
            ...
            artifact_rel = rel.removeprefix(".dingllm/")
            return self._artifact_persistence.save_artifact(
                project_path, artifact_rel, content
            )
